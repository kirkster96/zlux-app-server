      ...
      manifest.appfwPlugins.forEach(function (manifestPluginRef) {
        ...
        if (pluginDefinition && initUtils.fileExists(pluginDefinition)) {
          if (enabled) {
            ...
            initUtils.registerPlugin(fullPath, pluginDefinition, config.pluginsDir, actionsPluginStorage, recognizersPluginStorage, RUNTIME_DIRECTORY);
          } else {
            initUtils.printFormattedDebug(`Deregistering plugin ${fullPath}`);
            initUtils.deregisterPlugin(pluginDefinition, config.pluginsDir, actionsPluginStorage);
          }
        } else {
